          ...
          <div className="defeat-shake" style={{
            ...
            <div style={{ marginBottom: "24px" }}>
              <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "12px", marginBottom: "10px" }}>
                인정받은 근거 ({unlockedClues.length}/4)
              </p>
              {unlockedClues.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  {CLUES.filter(c => unlockedClues.includes(c.id)).map(c => (
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#c8a96e" }}>
                      <span>✓</span><span>{c.label}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "12px" }}>인정받은 근거가 없습니다.</p>
              )}
            </div>
            <button onClick={handleReset} style={{
